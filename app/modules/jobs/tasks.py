...
async def aggregate_daily_analytics():
    """Aggregate yesterday's metrics into daily_analytics_summary (per store)."""
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            for store in stores:
                camera_ids_q = select(Camera.id).where(Camera.store_id == store.id)
                camera_ids = [r for r in (await db.execute(camera_ids_q)).scalars().all()]
                if not camera_ids:
                    continue

                # Footfall: entry line crossings
                total_footfall = (
                    await db.execute(
                ...
                unique_visitors = (
                    await db.execute(
                        select(func.count(func.distinct(TrackSession.person_identity_id))).where(
                            TrackSession.camera_id.in_(camera_ids),
                ...
                total_billing = (
                    await db.execute(
                        select(func.count()).where(
                            BillingInteraction.camera_id.in_(camera_ids),
                ...
                existing = (
                    await db.execute(
                        select(DailyAnalyticsSummary).where(
                            DailyAnalyticsSummary.store_id == store.id,
            ...
            await db.commit()
            logger.info(f"Daily analytics aggregated for {yesterday} ({len(stores)} stores)")
