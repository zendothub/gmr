...
async def reload_config(current_user: User = Depends(get_current_user)):
...
@router.post("/start")
async def start_runtime(current_user: User = Depends(get_current_user)):
    """Start workers for all cameras marked active in the database."""
    supervisor = WorkerSupervisor.get_instance()
    try:
...
@router.post("/stop")
async def stop_runtime(current_user: User = Depends(get_current_user)):
